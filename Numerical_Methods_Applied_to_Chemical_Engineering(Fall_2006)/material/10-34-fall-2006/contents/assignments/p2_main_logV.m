...
% This implementation uses a scaled variable LN(V_rxtr) as well as the
% other normal variables.  

function p2_main_logV
clear; clc;
...
% Choose the Volume variable to be LN(V)
var0 = [C_Ao_guess; C_Bo_guess; T_guess; log(V_rxtr_guess)];

% note that in the scaled case, you do not very high tolerances.  The
% tolerance in this case only serves to increase the precision of the
% solution (even default tolerance will get you very close)
...
disp(['Reactor Volume = ',num2str(exp(var(4))),' L']);
...
t_span = [0, 1e5];
[time, C_all] = ode45(@rxtr_dCdt_ode,t_span,zeros(8,1),[],C_Ao,C_Bo,T,V_rxtr,V_flow,Af,EaR);
...
for i=1:length(test)
cost_i(i) = obj_fun([var(1:3);log(test(i))],V_flow,Af,EaR);
end
...
function cost = obj_fun(var,V_flow,Af,EaR)
    ...
    V_rxtr = exp(var(4)); % since we are using LN(V) as the variable
    ...
    % the cost function in the negative of [C]final
    cost = -(C_all(end,3));
...
%  all of the C variable are concentrations in moles/L
    ...
    k = Af.*exp(-EaR./T);  % calculates the rate constants
