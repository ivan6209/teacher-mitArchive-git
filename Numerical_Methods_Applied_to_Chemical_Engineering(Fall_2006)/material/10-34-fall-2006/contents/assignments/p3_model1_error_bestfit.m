function [params,res] = model1_error_bestfit()
...
%estimates
k1 = 0.0497; k2 = 0.0889; Kb1 = 0.3; Kb2 = 0.0501; 
...
params = fminsearch(@S_error, params0,options,S);
...
for i=1:length(time)
    cb(i) = exp(-k1*time(i));
    cc(i) = (1 + (k2*exp(-k1*time(i)))/(k1-k2) + (k1*exp(-k2*time(i)))/(k2-k1));
end
...
for i=1:sizeS(1)
    for j = 1:sizeS(2)
        res(i*(sizeS(2)-1)+j) = S(i,j) - S_model(i,j);
    end
end
...
%calculate the normally distributed errors and store them in norm.
norm= zeros(1,40);
for i=2:length(x)
